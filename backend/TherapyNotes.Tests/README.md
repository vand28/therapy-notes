...
## Overview
This test suite covers the core functionality of the TherapyNotes SaaS application, including authentication, usage limits, client management, parent features, and data models.

## Test Project Structure

```
backend/TherapyNotes.Tests/
  ...
  test:
    ...
    steps:
      ...
      - name: Restore dependencies
        run: dotnet restore backend/TherapyNotes.Tests
      - name: Build
        run: dotnet build backend/TherapyNotes.Tests
      - name: Test
        run: dotnet test backend/TherapyNotes.Tests --no-build --verbosity normal
```
...
### Run Single Test
```bash
dotnet test --filter "FullyQualifiedName=TherapyNotes.Tests.AuthServiceTests.RegisterAsync_ValidRequest_CreatesUser"
```
...
```json
{
  ...
  "preLaunchTask": "build",
  "program": "dotnet",
  "args": ["test", "${workspaceFolder}/backend/TherapyNotes.Tests"],
