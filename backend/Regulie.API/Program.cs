...
using Regulie.Infrastructure.Storage;
...
var mongoDbName = builder.Configuration["MongoDB:DatabaseName"] ?? "regulie";
...
var storageBucketName = builder.Configuration["Storage:BucketName"] ?? "regulie";
...
var fromEmail = builder.Configuration["Resend:FromEmail"] ?? "Regulie <noreply@regulie.com>";
...
builder.Services.AddSingleton<IGoogleAuthService>(new Regulie.Infrastructure.Auth.GoogleAuthService(googleClientId));

// Register MFA service
builder.Services.AddSingleton<IMfaService, Regulie.Infrastructure.Auth.MfaService>();
...
builder.Services.AddSingleton(
    new Regulie.Infrastructure.Payments.StripePaymentService(stripeSecretKey, stripeWebhookSecret, stripeProfessionalPriceId, stripePremiumPriceId)
);

// Register Email service (optional - only if API key is configured)
if (!string.IsNullOrEmpty(resendApiKey))
{
    builder.Services.AddSingleton(new Regulie.Infrastructure.Email.ResendEmailService(resendApiKey, fromEmail));
}

// Register Report service
builder.Services.AddSingleton<Regulie.Infrastructure.Reports.ReportService>();
