...
This guide covers deploying Regulie using Docker containers.
...
docker tag regulie-backend:latest <account-id>.dkr.ecr.us-east-1.amazonaws.com/regulie-backend:latest
docker push <account-id>.dkr.ecr.us-east-1.amazonaws.com/regulie-backend:latest
```
...
```bash
# Build and push
gcloud builds submit --tag gcr.io/PROJECT-ID/regulie-backend backend/
gcloud builds submit --tag gcr.io/PROJECT-ID/regulie-frontend frontend/

# Deploy
gcloud run deploy regulie-backend \
  --image gcr.io/PROJECT-ID/regulie-backend \
  --platform managed \
  --region us-central1 \
  --allow-unauthenticated

gcloud run deploy regulie-frontend \
  --image gcr.io/PROJECT-ID/regulie-frontend \
  --platform managed \
  --region us-central1 \
  --allow-unauthenticated
```
...
# Send to CloudWatch
awslogs get /docker/regulie-backend ALL
```
...
docker tag regulie-backend:old regulie-backend:latest
...
# Inspect container
docker inspect regulie-backend
```
...
```bash
# Check network
docker network inspect regulie-network
...
# Cache from registry
docker-compose build --cache-from regulie-backend:latest
```
